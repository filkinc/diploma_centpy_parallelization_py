...
\end{frame}
	...
	\begin{frame}{Одномерная постановка задачи}
		...
		\vspace{-0.2 cm}
		Зона пониженной плотности при $\alpha = 0.1$ моделирует энерговложение; \\ Слева --- ГУ Неймана, справа --- ГУ непротекания.
	\end{frame}
...
%	\end{frame}
	...
	%==================================================================
	\begin{frame}{Анализ сходимости относительно точного решения}
		...
		\begin{columns}[c]
			...
			\begin{column}{0.48\textwidth}
				...
				\begin{table}[h]
					\centering
					\caption{\smaller{Эмпирический порядок $p$ ($N{:}\,200{\to}400$)}}
					\vspace{-0.2cm}
...
%	\end{frame}
	...
	\begin{frame}{Сравнительный анализ аппаратного ускорения: одномерная постановка}
		...
		\begin{columns}[c]
			...
			\begin{column}{0.58\textwidth}
				...
				\begin{table}[h]
					\centering
					\caption{\smaller{Время счёта (с), 1D, схемы SD2 и FD2}}
					\vspace{-0.3 cm}
					\resizebox{\textwidth}{!}{%
						\begin{tabular}{lcccccccc}
							\hline
							& \multicolumn{2}{c}{\texttt{centpy}}
							& \multicolumn{2}{c}{JAX CPU}
							& \multicolumn{2}{c}{JAX GPU}
							& \multicolumn{2}{c}{JAX-FL.} \\
							$J$ & SD2 & FD2 & SD2 & FD2 & SD2 & FD2 & CPU & GPU \\
							\hline
							$\mathbf{256}$   & 1.138   & 0.218   & 0.096   & 0.023  & \textbf{0.143}  & \textbf{0.131} & 0.954   & 3.894   \\
							$\mathbf{512}$   & 4.244   & 0.662   & 0.394   & 0.057  & \textbf{0.179}  & \textbf{0.189} & 2.857   & 7.545   \\
							$\mathbf{1024}$  & 5.577   & 1.972   & 2.499   & 0.207  & \textbf{0.370}  & \textbf{0.280} & 4.893   & 16.403  \\
							$\mathbf{2048}$  & 16.175  & 3.875   & 7.255   & 0.785  & \textbf{0.720}  & \textbf{0.588} & 13.040  & 33.451  \\
							$\mathbf{4096}$  & 41.613  & 13.218  & 29.928  & 4.067  & \textbf{1.500}  & \textbf{1.217} & 44.688  & 70.443  \\
							$\mathbf{8192}$  & 152.440 & 46.947  & 86.862  & 14.377 & \textbf{3.931}  & \textbf{3.082} & 161.556 & 153.630 \\
							$\mathbf{16384}$ & 612.826 & 186.817 & 349.892 & 60.462 & \textbf{12.805} & \textbf{8.819} & 633.204 & 404.548 \\
							\hline
						\end{tabular}
					}
				\end{table}
				...
				\begin{table}[h]
					...
					\resizebox{0.85\textwidth}{!}{%
						\begin{tabular}{lcc}
							...
							Сравнение & SD2 & FD2 \\
							\hline
							\texttt{JAX} CPU        & 27.3$\times$          & 6.9$\times$  \\
							\texttt{centpy}        & \textbf{47.9$\times$} & 21.2$\times$ \\
							\texttt{JAX-FLUIDS} GPU & 31.6$\times$          & 45.9$\times$ \\
							\hline
						\end{tabular}
					}
				\end{table}
	...
	\begin{frame}{Двумерная постановка задачи}
		...
		\vspace{-0.2 cm}
		Зоны пониженной плотности при $\alpha = 0.1$ --- круговая (центр $(0.45, 0.65)$, радиус $0.08$) и прямоугольная ; Слева  --- ГУ Неймана, Остальные --- ГУ непротекания.
...
%	\end{frame}
	...
	\begin{frame}{Сходимость в 2D: изоэнтропийный вихрь Ху--Шу}
		...
		\begin{figure}[h]
			...
			\vspace*{-0.2cm}
			\caption{\smaller{Ошибка по p,u,v,$rho$, L1, 2D}}
		\end{figure}
...
%				\vspace{0.5cm}
				\textbf{Тест:} перенос гладкого изоэнтропийного вихря Ху--Шу
				($\varepsilon = 5$) на области $[0,10]^2$ с периодическими ГУ;
				неограниченная реконструкция, $t = 1$.
%				\vspace{0.15cm}
%				\begin{alertblock}{}
%					\smaller \textbf{2-й порядок по всем переменным} и по обеим
%					нормам. В отличие от 1D, $L_2$ \emph{не проседает} --- гладкий
	...
	\begin{frame}{Двумерная постановка: эволюция плотности}
		...
		\begin{columns}[c]
			...
			\begin{column}{0.22\textwidth}
				...
				\textbf{\small Начальное условие}\par
				\vspace{0.2cm}
				\hspace*{-0.4cm}% <-- сдвиг графика влево
		...
		\begin{columns}[c]
			\begin{column}{0.42\textwidth}
				\begin{figure}[h]
					...
					\caption{\smaller{Время счёта (с) от размера сетки $J\times J$, 2D}}
				\end{figure}
			\end{column}
			\begin{column}{0.58\textwidth}
				% ===== Основная таблица: SD2 + FD2 =====
				...
				\begin{table}[h]
					...
					\resizebox{0.85\textwidth}{!}{%
						\begin{tabular}{lcc}
							...
							Сравнение & SD2 & FD2 \\
							\hline
							\texttt{JAX} CPU        & 33.2$\times$          & \textbf{59.6$\times$}  \\
							\texttt{centpy}        & 91.5$\times$          & \textbf{102.0$\times$} \\
							\texttt{JAX-FLUIDS} GPU & 2.5$\times$           & 5.9$\times$            \\
							\hline
						\end{tabular}
					}
				\end{table}
...
%	\end{frame}
	...
	%==================================================================
	% Appendix: определения норм погрешности
	\begin{frame}{Нормы погрешности $L_1$ и $L_2$}
		
		Погрешность численного решения: $e_i = u_i^{\,h} - u_i^{\,\text{exact}}$,
		где $u^h$ --- численное, $u^{\text{exact}}$ --- точное решение.
		
		\vspace{0.3cm}
		\begin{block}{Непрерывные нормы на области $\Omega$}
			\[
			\|e\|_{L_1} = \int_\Omega |e|\,d\Omega,
			\qquad
			\|e\|_{L_2} = \left( \int_\Omega |e|^2\,d\Omega \right)^{1/2}
			\]
		\end{block}
		
		\begin{block}{Дискретные (сеточные) аналоги}
			Для равномерной сетки из $N$ ячеек (1D, мера ячейки $\Delta x$):
			\[
			\|e\|_{L_1} = \Delta x \sum_{i=1}^{N} |e_i|,
			\qquad
			\|e\|_{L_2} = \left( \Delta x \sum_{i=1}^{N} |e_i|^2 \right)^{1/2}
			\]
			Для двумерной сетки $N\times N$ (мера ячейки $\Delta x\,\Delta y$):
			\[
			\|e\|_{L_1} = \Delta x\,\Delta y \!\sum_{i,j} |e_{ij}|,
			\qquad
			\|e\|_{L_2} = \Big( \Delta x\,\Delta y \!\sum_{i,j} |e_{ij}|^2 \Big)^{1/2}
			\]
		\end{block}
		
		\begin{itemize}
			\item[$\bullet$] {\small $L_1$ --- средняя по модулю погрешность; \enspace
				$L_2$ --- среднеквадратичная, \emph{сильнее} штрафует крупные локальные ошибки.}
		\end{itemize}
