...
\end{frame}
	...
	\begin{frame}{Одномерная постановка задачи}
		...
		\begin{block}{Система уравнений Эйлера}
			...
			\partial_x \begin{bmatrix} \rho u \\ \rho u^2 + p \\ (\rho E + p)u \end{bmatrix} = 0
			\]
		...
		\vspace{-0.2 cm}
		Зона пониженной плотности ($\alpha = 0.1$) моделирует энерговложение; ударная волна ($M_s = 2$) движется справа налево. На краях --- граничные условия Неймана.
	\end{frame}
	...
	\begin{frame}{Численное решение: одномерная задача}

		\begin{figure}[!h]
			\begin{minipage}[t]{0.33\textwidth}
				\centering
				\includegraphics[width=1.06\textwidth]{rieman_modify_t0.png}
				\caption*{\smaller{(a)}}
			\end{minipage}\hfill
			\begin{minipage}[t]{0.33\textwidth}
				\centering
				\includegraphics[width=\textwidth]{rieman_modify_t_half.png}
				\caption*{\smaller{(b)}}
			\end{minipage}\hfill
			\begin{minipage}[t]{0.33\textwidth}
				\centering
				\includegraphics[width=\textwidth]{rieman_modify_t_final.png}
				\caption*{\smaller{(c)}}
			\end{minipage}
			\vspace{-0.3 cm}
			\caption{ \smaller{Эволюция примитивных переменных в модифицированной 1D задаче Римана: (a) $t = 0$; (b) $t = 0.125$; (c) $t = 0.25$ с наложением на эталон \texttt{JAX-FLUIDS}}}
		\end{figure}
	...
	\begin{frame}{Численное решение: двумерная задача}

		\begin{columns}[c]
			\begin{column}{0.62\textwidth}
				\begin{figure}[h]
					\centering
					\includegraphics[width=\textwidth]{rho_modify_rieman_plot_2d_sd2.png}
					\vspace{-0.2 cm}
					\caption{\smaller{Поле плотности $\rho(x,y)$, схема SD2: $t=0$; $t=0.125$; $t=0.25$}}
				\end{figure}
			\end{column}
			\begin{column}{0.38\textwidth}
				\textbf{Двумерная картина течения:}
				\begin{itemize}
					\item грибовидная вихревая структура за фронтом УВ;
					\item неустойчивость \textbf{Рихтмайера--Мешкова} на контактной границе;
					\item профили согласуются с \texttt{JAX-FLUIDS}.
				\end{itemize}
				\vspace{0.2cm}
				\begin{alertblock}{}
					\small Сетки до $1024\times1024$ достижимы только на GPU.
				\end{alertblock}
			\end{column}
		\end{columns}
	...
	\begin{frame}{Сравнительный анализ аппаратного ускорения}
