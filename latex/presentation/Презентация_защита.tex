...
\begin{frame}{Библиотека JAX --- инструмент реализации}
	...
	\begin{columns}[t]
		\begin{column}{0.44\textwidth}
			\begin{block}{Что такое JAX}
				JAX --- \textbf{современная библиотека} Google для высокопроизводительных вычислений и машинного обучения; NumPy-совместимый, JIT-компиляция.
			\end{block}
			\begin{block}{Почему именно JAX}
				Параметрическое исследование по $\alpha$ требует \textbf{множества быстрых расчётов}. JAX даёт скорость компилируемого кода при простоте Python, а также \textbf{единый код} CPU/GPU.
			\end{block}
		\end{column}
...
%	\end{frame}
	...
	\begin{frame}{Численное решение: одномерная постановка}
		...
		\begin{figure}[!h]
			...
			\end{minipage}
			\vspace{-0.5 cm}
			\caption{ \smaller{Эволюция примитивных переменных в модифицированной 1D задаче Римана: (a) Плотность; (b) Скорость; (c) Давление с наложением на эталон JAX-FLUIDS}}
		\end{figure}
	...
	%==================================================================
	\begin{frame}{Анализ сходимость в одномерной постановке}
...
%	\end{frame}
	...
	\begin{frame}{Анализ аппаратного ускорения: одномерная постановка}
		...
		% TODO отделить наш результат от остальных
				...
				\begin{table}[h]
					...
					\resizebox{\textwidth}{!}{%
						\begin{tabular}{lcccccccc}
							...
							$\mathbf{8192}$\tikzmark{br}  & 152.440 & 46.947  & \textbf{86.862}  & 14.377 & \textbf{3.931}  & \textbf{3.082} & 161.556 & 153.630 \\
...
%					\vspace{-0.3 cm}
					\resizebox{0.85\textwidth}{!}{%
						\begin{tabular}{lcc}
							...
							\tikzmark{tlc} \texttt{centpy} & \textbf{47.9$\times$} & \textbf{21.2$\times$} \tikzmark{brc} \\
	...
	\begin{frame}{Двумерная постановка задачи}
		...
		в расчётной области $(x, y) \in [0, 1]^2$, с кусочно-постоянными НУ
		\vspace{-0.15 cm}
		\[
		(\rho, u, v, p)|_{t=0} =
		\begin{cases}
...
%	\end{frame}
	...
	\begin{frame}{Анализ сходимости в двумерной постановке}
		...
		\begin{figure}[h]
			...
			\vspace*{-0.2cm}
			%
			\caption{\smaller{Ошибка по p,u,v,$\rho$, норма L1, 2D}}
		\end{figure}
				...
				\begin{table}[h]
					...
					\resizebox{0.85\textwidth}{!}{%
						\begin{tabular}{lcc}
							...
							Сравнение & SD2 & FD2 \\
							\hline
							\texttt{JAX} CPU        & 33.2$\times$          & 59.6$\times$  \\
							\tikzmark{tlc2} \texttt{centpy}  & \textbf{91.5$\times$ }         & \textbf{102.0$\times$} \tikzmark{brc2} \\
...
\begin{frame}{Нормы погрешности $L_1$, $L_2$ и порядок сходимости}
	...
	\vspace{0.2cm}
	\begin{block}{Норма $L_1$ --- погрешность интегрального характера}
		\[
	...
	\begin{block}{Норма $L_2$ --- среднеквадратическая (равномерность погрешности)}
		\[
...
\begin{frame}
	\begin{block}{Как работает JAX (JIT-конвейер)}
		\small
		\begin{enumerate}\itemsep2pt
			\item \textbf{Трассировка:} JAX проходит по Python-функции с абстрактными аргументами и записывает все операции.
			\item \textbf{Граф (\texttt{jaxpr}):} из трассы строится промежуточное представление --- граф вычислений.
			\item \textbf{Компиляция XLA:} граф транслируется в оптимизированное машинное ядро под устройство (CPU/GPU/TPU).
			\item \textbf{Исполнение и кэш:} ядро запускается; повторные вызовы берут готовую компиляцию из кэша.
		\end{enumerate}
		%				\vspace{0.1cm}
